...
async function recordResult(lobby, outcome) {
  ...
  await hset(`player:${bL}`, { name: bN });
  await hset(`player:${wL}`, { name: wN });
...
export async function createLobby(playerId, name) {
  let code = randomCode();
...
export async function joinLobby(playerId, name, code) {
