...
// A five only counts with a non-friendly cell on BOTH sides -- six in a row is
// not a win, so it must not score like one. That boundary is why the scoring
// window is 7 cells wide rather than 6.
const PATTERNS = [
...
function isFive(st, idx, color) {
  ...
  for (const [dr, dc] of DIRS) {
    ...
    for (let k = 1; codeOf(st.b, r - dr * k, c - dc * k, color) === 1; k++) run++;
    if (run === 5) return true; // exactly five; an overline wins nothing
  }
  return false;
}
  ...
  for (const [dr, dc] of DIRS) {
    for (let o = -4; o <= 0; o++) {
      ...
      if (ok && ones === 4 && gap >= -4) {
        ...
        // Confirm it really wins: filling the gap may make six, which is nothing.
        st.b[p] = color;
        const wins = isFive(st, p, color);
        st.b[p] = 0;
        if (wins) pts.push(p);
      }
    }
