...
// Instrumentation for playtests: counts moves that came from the threat search.
export const STATS = { vct: 0 };
...
function ab(st, depth, alpha, beta, maxing, ai, ply, deadline, branch) {
  ...
  const verify = maxing ? st.h2 : (st.h2 ^ SIDE_KEY);
  const alpha0 = alpha, beta0 = beta;
  if (ttOn && TT_GEN[slot] === ttGen && TT_KEY[slot] === verify && TT_DEPTH[slot] >= depth) {
...
export const _internals = { STATS, newState, place, unplace, evalFor, vcf, winMove, completions, forbidden };
