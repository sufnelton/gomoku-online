import { SIZE, DIRS, other, makesDoubleThree } from "./gomoku.js";
...
const W = (() => {
  const t = new Float64Array(729);
  for (let i = 0; i < 729; i++) {
    let s = "", x = i;
    for (let k = 0; k < 6; k++) { s += x % 3; x = (x / 3) | 0; }
    let w = 0;
...
const RAW = new Int8Array(11); // scratch: offsets -5..+5 along one direction

// Sum of W over the six windows through offset 5 of RAW, scored for `me`.
// Window j spans RAW[j..j+5]; the index rolls instead of being rebuilt.
function windowSum(me) {
  let idx = 0, p = 1;
  for (let k = 0; k < 6; k++) { idx += asCode(RAW[k], me) * p; p *= 3; }
  let sum = W[idx];
  for (let j = 1; j <= 5; j++) {
    idx = (idx - asCode(RAW[j - 1], me)) / 3 + asCode(RAW[j + 5], me) * 243;
    sum += W[idx];
  }
  return sum;
}
...
const loadLine = (b, r, c, dr, dc) => {
  for (let i = 0; i < 11; i++) RAW[i] = rawAt(b, r + (i - 5) * dr, c + (i - 5) * dc);
};
...
function gains(st, idx) {
  ...
  for (let d = 0; d < 4; d++) {
    ...
    RAW[5] = 1; GAIN[1] += windowSum(1) - b1;
    RAW[5] = 2; GAIN[2] += windowSum(2) - b2;
    RAW[5] = 0;
  }
  return GAIN;
}
...
function forbidden(st, idx, color) {
  ...
  st.b[idx] = color;
  const bad = makesDoubleThree((r, c) => codeOf(st.b, r, c, color), (idx / SIZE) | 0, idx % SIZE);
  st.b[idx] = 0;
  return bad;
}
