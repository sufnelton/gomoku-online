...
describe("blocking exemption", () => {
  ...
  it("allows it when it blocks a five", () => {
    const b = doubleThreeSetup();
    put(b, "white", [[3, 3], [4, 4], [5, 5], [6, 6]]); // white four; (7,7) completes five
    expect(isForbidden(b, 7, 7, "black")).toBe(false);
  });

  it("still forbids it when it only blocks a four", () => {
    const b = doubleThreeSetup();
    put(b, "white", [[4, 4], [5, 5], [6, 6]]); // (7,7) would only give white a four
    expect(isForbidden(b, 7, 7, "black")).toBe(true);
  });

  it("still forbids it when it only blocks an open three", () => {
    const b = doubleThreeSetup();
    put(b, "white", [[5, 5], [6, 6]]); // (7,7) would give white only an open three
    expect(isForbidden(b, 7, 7, "black")).toBe(true);
  });

  it("allows it at either end of an open four", () => {
    // Both ends of an open four are five-making points, so both are real blocks.
    const b = doubleThreeSetup();
    put(b, "white", [[3, 3], [4, 4], [5, 5], [6, 6]]);
    expect(isForbidden(b, 7, 7, "black")).toBe(false);
  });
  ...
  it("drops the point from forbiddenPoints once it becomes a block", () => {
    ...
    put(b, "white", [[3, 3], [4, 4], [5, 5], [6, 6]]); // (7,7) now stops a five
