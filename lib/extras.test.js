...
describe("passphrase gate", () => {
  ...
  it("never sends the passphrase to a client", async () => {
    ...
    // A lobby code is guessable, so there is no longer a read that a stranger
    // can do at all -- which is the only thing that ever made the gate real.
    expect(await getState(created.code, "a-stranger")).toEqual({ error: "not_a_player" });

    // And a player's own read still carries no passphrase.
    const seen = await getState(created.code, P1);
