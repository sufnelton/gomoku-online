...
export function findWin(board, r, c) {
  ...
  for (const [dr, dc] of DIRS) {
    ...
    if (line.length === 5) return line; // exactly five; an overline is not a win
  }
  return null;
}
...
export function makesDoubleThree(codeAt, r, c) {
  ...
  for (const [dr, dc] of DIRS) {
    ...
    for (let k = 1; codeAt(r - dr * k, c - dc * k) === 1; k++) run++;
    if (run === 5) return false; // exactly five wins outright, so it is always legal
  }
  let threes = 0;
  for (const [dr, dc] of DIRS) {
...
/* Would the OPPONENT playing this same point make a four or a five there?
 * Only that counts as a block. Answering a mere open three does not buy you
 * a double three -- you have to deal with those somewhere else. */
export function blocksThreat(codeAt, r, c) {
  for (const [dr, dc] of DIRS) {
    for (let o = -4; o <= 0; o++) {
      let ones = 0, ok = true;
      for (let k = 0; k < 5; k++) {
        const v = codeAt(r + (o + k) * dr, c + (o + k) * dc);
        if (v === 1) ones++;
        else if (v !== 0) { ok = false; break; }
      }
      if (ok && ones >= 4) return true;
    }
  }
  return false;
}
