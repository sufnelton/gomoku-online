...
export function makesDoubleThree(codeAt, r, c) {
  ...
  for (const [dr, dc] of DIRS) {
    ...
    for (let k = 1; codeAt(r - dr * k, c - dc * k) === 1; k++) run++;
    if (run >= 5) return false; // five wins outright
  }
  let threes = 0;
  for (const [dr, dc] of DIRS) {
...
/* Would the OPPONENT playing this same point gain a threat you must answer?
 * A four (or five) forces a reply; an open three becomes an open four next
 * move. Anything less isn't a block, it's an attack. */
export function blocksThreat(codeAt, r, c) {
  ...
  for (const [dr, dc] of DIRS) if (isOpenThreeDir(codeAt, r, c, dr, dc)) return true;
  return false;
}
