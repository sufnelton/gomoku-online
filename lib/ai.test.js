...
describe("incremental evaluation", () => {
  ...
  /* The gain cache keys on a hash of the stones that can reach a cell, so a
   * place/unplace round-trip has to leave a sibling's cached score valid AND
   * correct. A stale hit here would not crash anything -- gains only drives
   * move ordering -- which is exactly why it needs its own test rather than
   * being trusted to show up in a strength run. */
  it("the gain cache returns what a cold computation would", () => {
    const { gains, dropGainCache } = _internals;
    const st = newState(randomBoard(3));
    const empty = [];
    for (let i = 0; i < SIZE * SIZE; i++) if (!st.b[i]) empty.push(i);

    const cold = (idx) => { dropGainCache(); const g = gains(st, idx); return [g[1], g[2]]; };

    // Warm the cache, then walk a search-shaped sequence: try a move, look at
    // the position under it, take it back, try the next one.
    for (const idx of empty.slice(0, 40)) gains(st, idx);
    for (const m of empty.slice(0, 12)) {
      for (const color of [1, 2]) {
        place(st, m, color);
        for (const idx of empty.slice(0, 40)) {
          if (idx === m) continue;
          const warm = gains(st, idx);
          const [w1, w2] = [warm[1], warm[2]];
          const [c1, c2] = cold(idx);
          expect(w1).toBeCloseTo(c1, 9);
          expect(w2).toBeCloseTo(c2, 9);
          for (const i2 of empty.slice(0, 40)) gains(st, i2); // re-warm
        }
        unplace(st, m);
      }
    }
