...
const PATTERNS = [
  ["0111110", 100000], ["0111112", 100000], ["2111110", 100000], ["2111112", 100000],
  ["011110", 15000],
...
describe("bot tactics", () => {
  ...
  for (const level of [3, 4, 5]) {
    it(`level ${level} makes exactly five instead of overshooting to six`, () => {
      // (7,6) would join the group into a run of six, which wins nothing.
      // (7,1) makes a clean five.
      const b = put(emptyBoard(), "white", [[7, 2], [7, 3], [7, 4], [7, 5], [7, 7]]);
      put(b, "black", [[10, 10], [11, 11]]);
      expect(chooseMove(b, level, "white", { budget: 400 })).toEqual([7, 1]);
    });
  }

  it("does not treat a six-making point as a five completion", () => {
    const b = put(emptyBoard(), "white", [[7, 2], [7, 3], [7, 4], [7, 5], [7, 7]]);
    const st = newState(b);
    const pts = _internals.completions(st, 7 * SIZE + 5, 2);
    expect(pts).toContain(7 * SIZE + 1);   // makes exactly five
    expect(pts).not.toContain(7 * SIZE + 6); // would make six
  });
  ...
  it("takes a double three when it is the only block on a four", () => {
    // (3,7) is a double three for black, but it also stops white's five, so
