...
describe("bot tactics", () => {
  ...
  it("the only block on a four can itself be an illegal double three", () => {
    // White's four needs (3,7). For black, (3,7) makes an open three up
    // column 7 and another down the (1,5)-(2,6) diagonal -> illegal.
    const b = emptyBoard();
    put(b, "white", [[3, 3], [3, 4], [3, 5], [3, 6]]);
    put(b, "black", [[3, 2], [1, 7], [2, 7], [1, 5], [2, 6]]);
    expect(isForbidden(b, 3, 7, "black")).toBe(true);
    const mv = chooseMove(b, 5, "black", { budget: 400 });
    expect(mv).not.toEqual([3, 7]);
  });
});
...
describe("bot strength", () => {
  ...
  // No budget override here: level 5's edge over level 4 is partly its bigger
  // thinking budget, so throttling both to the same clock erases what we test.
  it("level 5 beats level 4 from both colors", () => {
    expect(playGame(5, 4, undefined)).toBe("black");
    expect(playGame(4, 5, undefined)).toBe("white");
  }, 300000);
});
